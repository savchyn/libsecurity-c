...
bool Pwd_GetHashedPwd(const unsigned char *caPwd, unsigned char hashPwd[HASH_PWD_LEN + 1]) {
  ...
  Utils_Free(orgPwd);
  if (!Utils_SetCharArrayLen(hashPwd, crypto_hash_BYTES) ) return false;
  return true;
}
...
STATIC bool isPwdValidHandler(const PwdS *pwd, const unsigned char *sPwd, bool isHashed) {
  ...
  if (!Utils_GetCharArrayLen(pwd->caSalt, &saltLen, MIN_SALT_LEN, MAX_SALT_LEN) ) {
    snprintf(errStr, sizeof(errStr), "Pwd_IsPwdValid: The given salt is not legal");
    return false;
  }
  if (isHashed) {
    if (!Utils_GetCharArrayLen(sPwd, &len, MIN_PASSWORD_LENGTH, MAX_PASSWORD_LENGTH) ) return false;
    Utils_CreateAndCopyUcString(&cahPwd, sPwd, len + UTILS_STR_LEN_SIZE);
  } else {
    len = strlen((const char *)sPwd);
    if (!generateSaltedHashPwd(sPwd, len, &(pwd->caSalt[UTILS_STR_LEN_SIZE]), saltLen, &cahPwd) ) {
      return false;
    }
  }
  for (i = 0; i < DEFAULT_NUMBER_OF_OLD_PASSWORDS; i++) {
    if (pwd->OldHashedPasswords[i] != NULL && Utils_CharArrayCmp(cahPwd, pwd->OldHashedPasswords[i])) {
...
STATIC bool generateSaltedHashPwd(const unsigned char *sPwd, int16_t pwdLen, const unsigned char *sSalt, int16_t saltLen, unsigned char **cahPwd) {
  ...
  if (!Utils_GenerateCharArray(sPwd, pwdLen, &caPwd) ) {
    return false;
  }
  if (!isPwdLengthValid(caPwd) ) {
    Utils_Free(caPwd);
    return false;
  }
  if (!Utils_GenerateCharArray(sSalt, saltLen, &caSalt) ) {
    Utils_Free(caPwd);
    return false;
  }
  if (!Salt_GenerateCharArraySaltedPassword(caPwd, caSalt, &genPwd) ) {
    Utils_Free(caPwd);
    Utils_Free(caSalt);
    return false;
  }
...
STATIC bool newUserPwdHandler(PwdS **newPwd, const unsigned char *sPwd, int16_t pwdLen, const unsigned char *sSalt, int16_t saltLen, PasswordStreangthType minPwdStrength, bool isPwdHashed) {
  ...
  if (isPwdHashed) {
    if (!Utils_GetCharArrayLen(sPwd, &len, MIN_PASSWORD_LENGTH, MAX_PASSWORD_LENGTH) ) {
      snprintf(errStr, sizeof(errStr), "newUserPwdHandler: The given hashed password is not legal");
      return false;
    }
    Utils_CreateAndCopyUcString(&cahPwd, sPwd, len + UTILS_STR_LEN_SIZE);
    if (!Utils_GetCharArrayLen(sSalt, &sLen, MIN_SALT_LEN, MAX_SALT_LEN) ) {
    ...
    if (!generateSaltedHashPwd(sPwd, pwdLen, sSalt, saltLen, &cahPwd) ) {
      Utils_Free(caSalt);
      return false;
    }
  }
...
STATIC bool updatePasswordHandler(PwdS *pwd, const unsigned char *sPwd, const unsigned char *sNewPwd, MicroSecTimeStamp expiration,
  ...
  if (!isPwdValidHandler(pwd, sPwd, isHashedPwd)  || !Pwd_IsPwdValid(pwd, sNewPwd) ) {
    return false;
  }
  if (isHashedPwd) {
    if (!Utils_GetCharArrayLen(sPwd, &len, MIN_PASSWORD_LENGTH, MAX_PASSWORD_LENGTH) ) {
    ...
    if (!generateSaltedHashPwd(sPwd, strlen((const char *)sPwd), &(pwd->caSalt[UTILS_STR_LEN_SIZE]), saltLen, &cahPwd) ) {
      snprintf(errStr, sizeof(errStr), "problem was found while verifing the new password");
      return false;
    }
  }
  if (!Utils_CharArrayCmp(cahPwd, pwd->hashPassword) ) {
  ...
  Utils_Free(cahPwd);
  if (!generateSaltedHashPwd(sNewPwd, strlen((const char *)sNewPwd), &(pwd->caSalt[UTILS_STR_LEN_SIZE]), saltLen, &cahPwd1) ) {
    snprintf(errStr, sizeof(errStr), "problem was found while verifing the new password");
    return false;
  }
  if (Utils_CharArrayCmp(cahPwd1, pwd->hashPassword)) {
...
bool Pwd_VerifyPassword(PwdS *pwd, const unsigned char *sPwd) {
  ...
  if (!Pwd_IsPwdValid(pwd, sPwd) ) return false;
  if (!Utils_GetCharArrayLen(pwd->caSalt, &saltLen, MIN_SALT_LEN, MAX_SALT_LEN) ) return false;
  if (!generateSaltedHashPwd(sPwd, strlen((const char *)sPwd), &(pwd->caSalt[UTILS_STR_LEN_SIZE]), saltLen, &cahPwd) ) return false;
  if (!Utils_CharArrayCmp(cahPwd, pwd->hashPassword) ) {
    Utils_Free(cahPwd);
    pwd->ErrorsCounter++;
...
STATIC bool storeOldPasswords(const PwdS *pwd, const SecureStorageS *storage, const char *prefix) {
  ...
  if (!Utils_IsPrefixValid("Pwd:storeOldPasswords", prefix) ) return false;
  ...
  if (!SecureStorage_AddItem(storage, (unsigned char *)key, strlen(key), (unsigned char *)val, strlen(val)) ) {
  ...
  Utils_Malloc((void **)(&key), prefixLen);
  for (i = 0; i < len; i++) {
    if (!Utils_GetCharArrayLen(pwd->OldHashedPasswords[i], &pwdLen, MIN_PASSWORD_LENGTH, MAX_PASSWORD_LENGTH) ) {
      Utils_Free(key);
      return false;
    }
    ...
    if (!SecureStorage_AddItem(storage, (unsigned char *)key, strlen(key), pwd->OldHashedPasswords[i], pwdLen + UTILS_STR_LEN_SIZE) ) {
      Utils_AddPrefixToErrorStr("Can't add item to storage, error");
      Utils_Free(key);
      return false;
    }
...
bool Pwd_Store(const void *pwd, const SecureStorageS *storage, const char *prefix) {
  ...
  if (!Utils_IsPrefixValid("Pwd_Store", prefix) ) return false;
  p = (const PwdS *)pwd;
  for (i = 0; i < NUM_OF_STRUCTS; i++) {
    ...
    if (!SecureStorage_AddItem(storage, (unsigned char *)key, strlen(key), (unsigned char *)pwdStr, strLen) ) {
...
STATIC bool loadOldPasswords(PwdS *pwd, const SecureStorageS *storage, const char *prefix) {
  ...
  if (!Utils_IsPrefixValid("Pwd:loadOldPasswords", prefix) ) return false;
  prefixLen = strlen(prefix) + strlen(PWD_OLD_PWD_LEN_PREFIX) + 1;
  Utils_Malloc((void **)(&key), prefixLen);
  snprintf(key, prefixLen, PWD_OLD_P_LEN_PREFIX_FMT, PWD_OLD_PWD_LEN_PREFIX, prefix);
  if (!SecureStorage_GetItem(storage, (unsigned char *)key, strlen(key), &val) ) {
...
bool Pwd_Load(void **loadPwd, const SecureStorageS *storage, const char *prefix, char **retName) {
  ...
  *retName = NULL;
  ...
  if (!Utils_IsPrefixValid("Pwd_Load", prefix) ) return false;
  for (i = 0; i < NUM_OF_STRUCTS; i++) {
    prefixLen = strlen(prefix) + strlen(PWD_PREFIX) + strlen(prefixStrVec[i]) + 1;
    Utils_Malloc((void **)(&key), prefixLen);
    snprintf(key, prefixLen, PWD_PREFIX_FMT, PWD_PREFIX, prefixStrVec[i], prefix);
  ...
  if (!newUserPwdHandler((PwdS **)loadPwd, pwd, pwdLen, salt, saltLen, STRENGTH_NIL, true) ) { // the loaded password is already hashed
    printf("Error: %s\n", errStr);
    Utils_Free(pwd);
    Utils_Free(salt);
    return false;
  ...
  if (!ret ) Pwd_FreeUserPwd(loadPwd);
  return ret;
}
...
bool Pwd_IsEqual(const void *pwdS1, const void *pwdS2) {
  ...
  for (i = 0; i < DEFAULT_NUMBER_OF_OLD_PASSWORDS; i++) {
    if ((pwd1->OldHashedPasswords[i] == NULL && pwd2->OldHashedPasswords[i] != NULL) ||
        ...
        !Utils_CharArrayCmp(pwd1->OldHashedPasswords[i], pwd2->OldHashedPasswords[i]) ) {
      ret = false;
      break;
    }
  }
